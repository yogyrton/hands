...
| photo_hero | string (path) | фото в шапке страницы услуги |
| photo_card | string (path) | фото в карточке на главной |
...
Связанные данные (рекомендация: JSON-колонки — проще для Filament Repeater; либо hasMany-таблицы):
...
|---|---|---|
| id | bigint PK | |
| user_id | bigint FK nullable | привязка к учётной записи для входа (см. 3) |
...
| bio1 | text | 1-й абзац |
| bio2 | text | 2-й абзац |
| photo_hero | string (path) | основной портрет (шапка страницы + карточка на главной) |
| sort_order | int | |
...
Связанные:
- **principles** — «Подход в работе»: список `{ title, description }` (3 шт). JSON или hasMany.
- **gallery** — до 3 фото (пути). Итого у мастера **3 фото**: 1 основное (`photo_hero`, идёт и на
  главную) + 2–3 в галерею на странице мастера.
...
Мастер-пользователь связан с профилем `Master` через `masters.user_id`.
...
| Посещения — создание/просмотр | ✅ (все) | ✅ (создание; просмотр — обсудить: свои или все) |
| Сертификаты — создание/использование/история | ✅ | ✅ |
| Отчёты: выручка, зарплаты | ✅ | 👁 своя сводка (кол-во/сумма/зарплата) |

Реализация в Filament: `Panel::authGuard()` + Policy на каждый Resource (`viewAny/view/create/update`
с проверкой `$user->role === 'admin'` или `$visit->master_id === $user->master->id`).

> **Открытый вопрос (нужно подтверждение):** мастер видит только свои посещения или все?
> Рекомендация: **свои** (проще, аккуратнее по данным). Легко ослабить позже.

> **Открытый вопрос:** отдельный логин на каждого мастера или один общий?
> Рекомендация: **отдельный на каждого** (корректная атрибуция посещений и зарплат).
...
Поля и поведение:
1. **Мастер** — выбор из активных (для залогиненного мастера можно предзаполнить/скрыть).
...
## 12. Открытые вопросы (подтвердить перед реализацией)

1. Мастер видит **свои** посещения или **все**? (рекоменд.: свои)
2. Логин: **отдельный** на мастера или **общий**? (рекоменд.: отдельный)
3. `includes/requests/details` и `principles/gallery` — **JSON-колонки** или **отдельные таблицы**?
   (рекоменд.: JSON — быстрее в Filament; таблицы — если понадобятся выборки/сортировки по элементам)
4. Настройки студии — **таблица `settings`** или **`config/studio.php`**? (рекоменд.: таблица + страница в админке)
5. Точные координаты студии для Яндекс-карты.

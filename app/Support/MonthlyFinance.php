 ...
 *   Выручка = деньги по визитам (касса). Продажи сертификатов в выручку НЕ идут
 *   (учтены отдельным блоком), чтобы не задваивать.
...
class MonthlyFinance
{
    ...
    public static function for(int $year, int $month): object
    {
        ...
        // Выручка = деньги по визитам (касса). Продажи сертификатов сюда НЕ входят:
        // серт при отхаживании даёт кассу отдельными платежами, а его полная
        // стоимость учтётся в зарплате мастера — двойного счёта быть не должно.
        // Сумму проданных сертификатов показываем отдельным справочным блоком.
        $revenueVisits = round(Visit::moneyRevenue($start, $end)['total'], 2);
        $revenueCerts = round(Certificate::soldTotal($start, $end), 2);
        $revenue = $revenueVisits;

 ...
 *   Выручка = касса по визитам (нал/безнал/доплаты) + продажи сертификатов месяца.
 *   Визиты по сертификату в кассе = 0 (деньги пришли при продаже), двойного счёта нет.
...
class MonthlyFinance
{
    ...
    public static function for(int $year, int $month): object
    {
        ...
        // Выручка = реально пришедшие деньги: касса по визитам (нал/безнал/доплаты)
        // + продажи сертификатов месяца. Визиты, отхоженные по сертификату, в кассе
        // дают 0 (деньги пришли при продаже серта), поэтому двойного счёта нет.
